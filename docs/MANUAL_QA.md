...
- Inspect Profiles in light appearance. Dark appearance and a standard packaged screenshot now pass with the real sidebar, header, profile rows, and actions rather than placeholder bars. New Profile opens its focused naming sheet and Cancel leaves profiles unchanged; creating and deleting a disposable profile remains a manual stateful check.
