...
A ready-to-install [GitHub Actions template](ci.yml.example) builds, tests, and compiles release mode with Xcode 26.3 on macOS 15. The connected GitHub token rejected workflow creation because it lacks the `workflow` scope, so CI is not enabled by this PR. A maintainer with that permission can copy the template to `.github/workflows/ci.yml`. Local build, test, UI inspection and optimized-build results are recorded in the PR. Hardware-specific scrolling feel and Apple system icon spacing are not inferred from unit tests.
