...
- The approved app identity is a modern notched MacBook with a sewn upper-right display repair. The status item uses Apple's native `laptopcomputer` template symbol so it remains crisp and adaptive at menu-bar size.
- The popover now presents four direct feature destinations with plain status text instead of repeated green checkmark capsules. Opening macMender from the popover always restores Overview and the full sidebar, including after Profiles was selected with the sidebar collapsed.
...
- Final source verification before the identity follow-up passed `swift build` and 97 tests in 10 suites. The identity/navigation follow-up expands the suite to 99 tests; its final gate is recorded in `docs/UI_REBRAND_QA.md`.
