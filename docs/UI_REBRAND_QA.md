...
| Profiles | Pass with Rename/Duplicate product gap — the packaged screen visibly renders the sidebar, header, selectable profile rows, current state, and actions. Selection does not implicitly activate a profile; Make Active, Create, and Delete wiring remain present. New Profile opens a focused naming sheet and Cancel leaves data unchanged. A regression check hid the sidebar, entered Profiles through Overview's in-content action, and confirmed the complete sidebar returned with all eight destinations. The former full-height collection hierarchy produced placeholder or blank pixels on this beta; one width-constrained scrolling page with lightweight selectable rows is the narrow presentation workaround. Rename and Duplicate are not implemented. |
...
| Check | Result / evidence |
| --- | --- |
| Dark appearance | Pass for captured screens — routine pages, onboarding, Option+Tab, the aligned quick-control popover, and the rebuilt Profiles page all render in the packaged app. |
| Light appearance | Partial — captured routine pages passed using a temporary ad-hoc QA copy forced to Aqua. Final Profiles, popover, and Option+Tab light capture remain manual. |
...
| Minimum size / large text | Standard 980×680 onboarding and scrollable permission layout passed. Minimum-practical-size onboarding remains manual. The compact 304×236 quick-control popover uses short labels and native switches; unusually large accessibility text and spoken ordering remain manual. |
...
- Profiles light-appearance confirmation, a stateful create/delete check with a disposable profile, plus a light-appearance Option+Tab capture.
