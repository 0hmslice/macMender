...
| Profiles | Partial — native Table, selection without implicit activation, Make Active, Create, and Delete wiring remain present and the AX tree is complete. A packaged regression check hid the sidebar, entered Profiles through Overview's in-content action, and confirmed the complete sidebar returned with all eight destinations. macOS 27 screen capture privacy-redacts this page while a text field is present, limiting visual automation. Rename and Duplicate are not implemented. |
